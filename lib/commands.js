...
/**
 * setDefaultWaitForTimeout(timeout, cb) -> cb()
 * sets the default waitFor call timeout, in milliseconds
 */
commands.setDefaultWaitForTimeout = function(){
  var cb = findCallback(arguments);
  var fargs = utils.varargs(arguments);
  this._defaultWaitForTimeout = fargs.all[0];
  cb();
};

/**
 * setDefaultWaitForPollFreq(pollFreq, cb) -> cb()
 * sets the default waitFor poll freq, in milliseconds
 */
commands.setDefaultWaitForPollFreq = function(){
  var cb = findCallback(arguments);
  var fargs = utils.varargs(arguments);
  this._defaultWaitForPollFreq = fargs.all[0];
  cb();
};
...
commands.waitFor = function(){
  ...
  // default
  opts.timeout = opts.timeout || (this._defaultWaitForTimeout || 1000);
  opts.pollFreq = opts.pollFreq || (this._defaultWaitForPollFreq || 200);
